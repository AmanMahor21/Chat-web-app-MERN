...
//   useEffect(() => {
//     if (!saveUser?._id) return; // Ensure `saveUser` exists and has an ID
...
//         // Process each message and update state
//         data.forEach((msg) => {
//           const { _id, createdAt, message } = msg;
//           const relevantId =
//             _id.sender === saveUser._id ? _id.receiver : _id.sender; // If the sender is the logged-in user, get the receiver
...
//           setFormatDate((prev = []) => {
//             const newData = [...prev];
//             const index = newData.findIndex((ele) => ele._id === relevantId);
...
//                 message,
//                 unreadCount: newData[index].unreadCount || 0,
//               };
//             } else {
//               newData.push({
//                 _id: relevantId,
...
//     fetchAllChat(); // Call the function within the effect
//   }, [saveUser, setFormatDate]); // Dependencies
// };
